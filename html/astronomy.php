...
if (isset($_GET['name']) AND isset($_GET['state']) AND isset($_GET['country'])) {
        if(($sname = $_GET['name']) && ($sstate = $_GET['state']) && ($scountry = $_GET['country'])) {
                $url = "http://api.wunderground.com/api/$key/astronomy/q/$scountry/$sstate/$sname.json";
                //echo $url;
                echo "";
...
else if (isset($_GET['name']) AND isset($_GET['state'])) {
	if(($sname = $_GET['name']) && ($sstate = $_GET['state'])) {
		$url = "http://api.wunderground.com/api/$key/astronomy/q/$sstate/$sname.json";
		//echo $url;
		echo "";
        $response = file_get_contents($url);
	//echo $response;
